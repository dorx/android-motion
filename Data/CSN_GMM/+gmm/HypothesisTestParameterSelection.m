classdef HypothesisTestParameterSelection
    
    % was GmLtStParameterSelection - evaluate model and Short-term,Long-temr feature parameters
    %
    ...
    methods(Static)
        ...
        function [AUC cTP cFP cParams models cThresholds] = evaluateParameters(data, parameters)
            ...
            %           directory of acsn files.
            %       sacTrainingRootDir 
            %       sacTestingRootDir - relative path from Picking to a directory
...
%             
            ...
            sacTrainingRootDir = data.sacTrainingRootDir;
            sacTestingRootDir = data.sacTestingRootDir;
            %
            ...
            sacTrainingRootDir = [rootDir sacTrainingRootDir];
            sacTestingRootDir = [rootDir sacTestingRootDir];
            
            assertDirectory(sacTrainingRootDir);
            assertDirectory(sacTestingRootDir);
            ...
            maxTrainingSegments = 5000;
            maxTestingSegments = 5000;
            ...
            % -------------------------------------------------------------
            % Create synthetic quake data
            
            
            maxSacTrainingRecords = 5000;
            
            %cSacTrainingRecords = SacLoader.loadSacRecords(sacTrainingRootDir);
            %cSacTestingRecords = SacLoader.loadSacRecords(sacTestingRootDir);
            
             load('*cache/sacTrainingRecords.mat');
             load('*cache/sacTestingRecords.mat');
            
            disp('Saving SAC records...')
            %save('*cache/sacTrainingRecords.mat', 'cSacTrainingRecords');
            %save('*cache/sacTestingRecords.mat', 'cSacTestingRecords');
            disp('Done saving.')
            
            % randomly subsample the training records
            
            % instead, create one synthetic segment for each sac onset:
            
            cSacTrainingRecords = cSacTrainingRecords(random.randomSubset(length(cSacTrainingRecords), maxSacTrainingRecords));
            
            cTrainingQuakeSegments= ...
                SegmentSynthesizer.createOnsetSegmentsAlternate(cTrainingSegments, cSacTrainingRecords, ...
                ltLength+deadTime, stLength, threshold, peakAmplitude, sampleRate);
            
            cSyntheticSegments= ...
                SegmentSynthesizer.createOnsetSegments(cTestingNormalSegments, cSacTestingRecords, ...
                ltLength+deadTime, stLength, threshold, peakAmplitude, sampleRate);
            ...
            parfor i=1:nParameters
                ...
                params.sigma = sigma;
                 ...
                 cTrainingQuakeFeatures = ...
                    GmLtStParameterSelection.computeTestingLtStFeatures({}, cTrainingQuakeSegments, ltLength, stLength, nFFT, nMoms, C, mu, sigma, dimensionsRetained);
                ...
                % learn GMM for normal data
                gm = GaussianMixture.learnGaussianMixture(cTrainingFeatures, nGaussians, 1); % <=== Magic Number
                
                % learn gm for quake examples
                gm_q = GaussianMixture.learnGaussianMixture(cTrainingQuakeFeatures, nGaussians, 1); % <=== Magic Number
                
                % evaluate training data
                %R = gm.evaluateProbability(cTestingFeatures) ./ gm_q.evaluateProbability(cTestingFeatures);
                %Assignments = -log(R);
                
                Assignments = -log(gm.evaluateProbability(cTestingFeatures)) + log(gm_q.evaluateProbability(cTestingFeatures));
        ...
        function cFeatures = ...
            ...
            cAnomalyBaseFeatures = GmLtStParameterSelection.computeLtStFeature(cAnomalySegments, ltLength, nFreq, nMom, stLength, nFreq, nMom, 1); % 2 is label of anomaly data
